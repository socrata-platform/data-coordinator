    ...
    withObjectFields(index, value) { accessor =>
      ...
      val streamType = command match {
        ...
        case "publish" =>
          val schemaHash = getOption[String]("schema")
          val dataVersion = getOption[Long]("data_version")
          PublishWorkingCopyMutation(index, datasetId, schemaHash, dataVersion)
        case "drop" =>
          val schemaHash = getOption[String]("schema")
          val dataVersion = getOption[Long]("data_version")
  ...
  private def runScript(u: Universe[CT, CV] with DatasetMutatorProvider with SchemaFinderProvider,
    ...
    try {
      ...
      commands.streamType match {
        ...
        case PublishWorkingCopyMutation(idx, datasetId, schemaHash, expectedDataVersion) =>
          mutator.publishCopy(user)(datasetId, checkDatasetState(idx, schemaHash, expectedDataVersion, _)).run(process(idx, datasetId, mutator))
    ...
    def carryOutCommand(commands: CommandStream, cmd: Command): Seq[MutationScriptCommandResult] = {
      ...
      def processCommand(cmd: Command): Seq[MutationScriptCommandResult] = cmd match {
        ...
        case SecondaryReindex(_) =>
          ...
          Seq(MutationScriptCommandResult.Uninteresting)
        case couid@CreateOrUpdateIndexDirective(idx, columnIdSpec, directive) =>
          columnIdSpec match {
            case Right(columnId) =>
            ...
            case Left(columnLabel) =>
              processCommand(couid.copy(id = Right(idFor(idx, columnLabel))))
          }
          Seq(MutationScriptCommandResult.Uninteresting)
        case did@DropIndexDirective(idx, columnIdSpec) =>
          columnIdSpec match {
            case Right(columnId) =>
              val columnOpt = mutator.schema.values.find(_.userColumnId == columnId)
              columnOpt.foreach(column => mutator.dropIndexDirective(column))
            case Left(columnLabel) =>
              processCommand(did.copy(id = Right(idFor(idx, columnLabel))))
          }
          Seq(MutationScriptCommandResult.Uninteresting)

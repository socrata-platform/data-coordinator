...
class PlaybackToSecondary[CT, CV](conn: Connection, secondaryManifest: SecondaryManifest, rowIdProcessor: RowIdProcessor, repFor: ColumnInfo => SqlColumnReadRep[CT, CV], timingReport: TimingReport) {
  ...
  def resync(datasetId: DatasetId, secondary: NamedSecondary[CV], delogger: Delogger[CV]) {
    timingReport("resync", "dataset" -> datasetId) {
      val w = new PostgresDatasetMapWriter(conn, rowIdProcessor.initial, timingReport)

...
trait Secondary[CT, CV] {
  ...
  /** Provide the current copy an update.  The secondary should ignore it if it
    ...
    */
  def version(datasetInfo: DatasetInfo, initialDataVersion: Long, finalDataVersion: Long, cookie: Cookie, events: Iterator[Event[CT, CV]], lastestDataVersion: () => Long): Cookie

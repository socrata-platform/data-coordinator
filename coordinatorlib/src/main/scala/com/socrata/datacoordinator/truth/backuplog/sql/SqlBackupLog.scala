...
class SqlBackupLog(conn: Connection) extends BackupLog {
  def findDatasetsNeedingBackup(limit: Int): Seq[BackupRecord] =
    using(conn.prepareStatement("SELECT dataset_system_id, latest_backup_data_version, latest_data_version FROM backup_log WHERE latest_data_version > latest_backup_data_version ORDER BY updated_at LIMIT ?")) { stmt =>
      stmt.setInt(1, limit)
  ...
  def completedBackupTo(datasetId: DatasetId, dataVersion: Long) {
    using(conn.prepareStatement("UPDATE backup_log SET latest_backup_data_version = ? WHERE dataset_system_id = ?")) { stmt =>

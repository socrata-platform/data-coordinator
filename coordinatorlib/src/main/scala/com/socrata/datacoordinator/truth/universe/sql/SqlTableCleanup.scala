...
class SqlTableCleanup(conn: Connection, daysDelay: Int = 1) extends TableCleanup { // daysDelay: Int = 1 will change to:  daysDelay: FiniteDuration
  ...
  private def cleanupDeleteds(): Unit = {
    ...
    val PurgedDatasetsExcludeOrphanCleanupInSecondary = PurgedDatasets + " AND not sm.pending_drop"
    ...
    using(conn.createStatement()) { stmt =>
      ...
      stmt.execute(s"DELETE FROM secondary_manifest WHERE dataset_system_id IN (${PurgedDatasetsExcludeOrphanCleanupInSecondary})")
    }
  }
}
